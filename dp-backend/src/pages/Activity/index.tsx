      ...
      getUserActivities(user.id, token)
        .then((activities) => {
          if ((activities as Transaction[]).length > 0) {
            ...
            const parsedRecords = orderedActivities.map((parsedActivity: Transaction) => {
              // Renombrar "date" a "dated" usando un type assertion
              const transformedActivity = {
                ...parsedActivity,
                dated: new Date((parsedActivity as any).date), // Ignorar la verificación de tipos temporalmente
              };
              return parseRecordContent(
                transformedActivity,
                RecordVariant.TRANSACTION
              );
            });
