      ...
      getUserActivities(user.id, token)
        .then((activities) => {
          if ((activities as Transaction[]).length > 0) {
            ...
            const parsedRecords = orderedActivities.map((activity: Transaction) => {
              // Renombrar "date" a "dated" usando un type assertion
              const transformedActivity = {
                ...activity,
                dated: new Date((activity as any).date), // Ignorar la verificación de tipos temporalmente
              };
              return parseRecordContent(transformedActivity, RecordVariant.TRANSACTION);
            });
