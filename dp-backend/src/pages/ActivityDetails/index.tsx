...
const ActivityDetails = () => {
  ...
  useEffect(() => {
    if (user && user.id) {
      getUserActivity(user.id, activityId, token)
        .then((activity) => {
          console.log('Objeto completo recibido del backend:', activity); // Inspecciona el objeto completo
  
          if (activity && activity.amount && activity.type) {
            // Renombrar la propiedad "date" a "dated"
            const transformedActivity = {
              ...activity,
              dated: (activity as any).date, // Ignora la verificación de tipos
            };
          
            console.log('Fecha transformada (dated):', transformedActivity.dated); // Verifica el valor aquí
  ...
  const formatDate = (dateString: string) => {
    try {
      ...
      return date.toLocaleString(); // Formatea la fecha al formato local
    } catch (error) {
          ...
          <div className="tw-border-neutral-blue-100 tw-rounded-lg print:tw-border-2 print:tw-p-48">
            ...
            <div className="tw-flex tw-flex-col tw-gap-y-6 tw-justify-center tw-mb-8 print:tw-mb-0">
              ...
              <div className="tw-flex tw-flex-col tw-gap-y-2 tw-items-center">
                ...
                <i>
                  {userActivity && formatDate(userActivity.dated)}
                </i>
              </div>
            </div>
          </div>
